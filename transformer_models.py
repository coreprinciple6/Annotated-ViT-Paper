...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels=3, patch_size=4, emb_size=64, img_size=32):
        ...
        self.projection = nn.Conv2d(in_channels, emb_size, kernel_size=patch_size, stride=patch_size) # Patch projection
        self.cls_token = nn.Parameter(torch.randn(1, 1, emb_size)) # Classification token
        self.positional_embedding = nn.Parameter(torch.randn(1, (img_size // patch_size) ** 2 + 1, emb_size)) # Positional embedding
...
class VisionTransformer(nn.Module):
    def __init__(self, in_channels=3, patch_size=4, emb_size=64,
                  img_size=32, num_classes=10, depth=7, num_heads=8,
                    forward_expansion=4, dropout=0.0):
        super().__init__()
